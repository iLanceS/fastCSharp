...
(function (fastCSharp) {
	var Pub = (function () {
		...
		Pub.LoadModule = function (Path) {
			if (this.IsModules[Path] == null)
				HttpRequest.AjaxGet(new HttpRequestQuery('pub.Error', { error: document.location.toString() + ' 加载了未知模块 ' + Path }));
			else {
		...
		Pub.LoadModuleWhenNull = function (Path, Version) {
			if (this.IsModules[Path] == null) {
				...
				this.AppendJs(fastCSharp.Loader.JsDomain + 'js/' + Path + '.js?v=' + Version);
			}
		};
		Pub.LoadView = function (View, IsReView) {
			if (!View)
				(View = new PageView).ErrorPath = 'http://127.0.0.1:8103/';
			if (View.ErrorPath) {
		...
		Pub.Load = function () {
			...
			if (fastCSharp.Loader.PageView) {
				...
				HttpRequest.AjaxGet(this.CreateLoadViewQuery());
			}
			else
			...
			setTimeout(this.ReadyFunction = this.ThisFunction(this, this.ReadyState), 1);
		};
	...
	var Events = (function () {
		...
		Events.prototype.Add = function (Function) {
			if (Function && this.Functions.indexOf(Function) < 0) {
		...
		Events.prototype.Remove = function (Function) {
			if (Function)
				this.Functions.RemoveAt(this.Functions.indexOf(Function));
			return this;
		};
	...
	var HttpRequest = (function () {
				...
				try {
					if (Request.status == 200 || Request.status == 304) {
						...
						if (Query.Url.substring(0, HttpRequest.ErrorPath.length) !== HttpRequest.ErrorPath) {
							HttpRequest.Ajax(new HttpRequestQuery('pub.Error', { error: '服务器请求失败 : ' + location.toString() + '\r\n' + Query.Url + (Query.SendString && !Query.FormData ? ('\r\n' + Query.SendString.length + '\r\n' + Query.SendString) : '') }));
						}
					}
				}
				finally {
		...
		HttpRequest.Ajax = function (HttpRequestQuery) {
		...
		;
		HttpRequest.AjaxGet = function (HttpRequestQuery) {
		...
		HttpRequest.MicrosoftXmlHttps = Pub.IE ? ['Microsoft.XMLHTTP', 'Msxml2.XMLHTTP'] : null; //['Microsoft.XMLHTTP','MSXML2.XMLHTTP','MSXML2.XMLHTTP.3.0','MSXML2.XMLHTTP.4.0','MSXML2.XMLHTTP.5.0']
		HttpRequest.AjaxGetRequest = new HttpRequest;
		HttpRequest.ErrorPath = '/ajax?n=pub.Error&';
		return HttpRequest;
	}());
	fastCSharp.HttpRequest = HttpRequest;
	Pub.AjaxCallBack = Pub.ThisFunction(HttpRequest.AjaxGetRequest, HttpRequest.AjaxGetRequest.CallBack);
				...
				if (Element.classList) {
					...
					if (OldName) {
						for (var Index = Names.length, OldNames = OldName.split(' '); --Index >= 0;)
							if (OldNames.indexOf(Names[Index]) < 0)
		...
		HtmlElement.$RemoveClass = function (Element, Names) {
			if (Names) {
				if (Element.classList) {
					...
					var OldName = Element.className;
					if (OldName) {
						for (var Index = Names.length, OldNames = OldName.split(' '); Index; OldNames.RemoveAtEnd(OldNames.indexOf(Names[--Index])))
							;
			...
			for (var Left = 0, Top = 0; Element != null && Element != document.body; Element = Element.offsetParent) {
				...
				if (Pub.IsFixed) {
					...
					if (Transform && Transform.indexOf('matrix(') != -1) {
						var XY = eval('HtmlElement.$Transform_' + Transform);
						if (XY.Left)
							Left += XY.Left;
						if (XY.Top)
							Top += XY.Top;
	...
	var SkinData = (function () {
		function SkinData(Parent, Name, Data, Function) {
			...
			this.$CanSet = (Parent == null || Parent.$CanSet) && Function == null;
		}
		...
		SkinData.prototype.$RemoveAt = function (Index, Count) {
			if (Count === void 0) { Count = 1; }
			var Data = this.$Data;
			if (Data instanceof Array && Index < Data.length && (Count = Data.splice(Index, Count).length)) {
	...
	var SkinDatas = (function () {
		...
		SkinDatas.prototype.Remove = function (SkinData) {
			this.Datas.RemoveAtEnd(this.Datas.indexOf(SkinData));
		};
	...
	var Skin = (function () {
		...
		Skin.prototype.EndHtml = function () {
			...
			this.Datas.length = 1;
			this.Htmls = null;
			return location.protocol == 'https:' ? Html.replace(/http\:\/\/127.0.0.1:8103\//g, 'https://127.0.0.1:8103/') : Html;
		};
	...
	window.onerror = function (message, filename, lineno, colno, error) {
		if (Pub.IE) {
			...
			if ((message != '语法错误' && message != 'Syntax error' && message != '語法錯誤') || Location.substring(0, filename.length) != filename) {
				var Error = navigator.appName + ' : ' + navigator.appVersion + '\r\n' + Location + '\r\n' + filename + ' [' + lineno + ',' + colno + '] ' + message;
				if (!Pub.Errors[Error]) {
					Pub.Errors[Error] = Error;
					HttpRequest.AjaxGet(new HttpRequestQuery('pub.Error', { error: Error }));
				}
			}
		}
		else {
