    ...
    public static class topologySort
    {
        ...
        private struct sorter<valueType>
        {
            ...
            private void pop(valueType point)
            {
                ...
                if (graph.TryGetValue(point, out points))
                {
                    if (points == null) log.Error.Throw("拓扑排序循环", new System.Diagnostics.StackFrame(), false);
                    graph[point] = null;
                    foreach (valueType nextPoint in points) pop(nextPoint);
                    graph.Remove(point);
                    values[--index] = point;
        ...
        public static valueType[] Sort<valueType>(ICollection<keyValue<valueType, valueType>> edges, HashSet<valueType> points
            , bool isDesc = false)
        {
            Dictionary<valueType, list<valueType>> graph = dictionary.CreateAny<valueType, list<valueType>>();
            list<valueType> pointList = null;
            if (edges != null)
            {
                pointList = new list<valueType>(edges.Count);
                ...
                foreach (keyValue<valueType, valueType> edge in edges)
                {
                    ...
                    pointList.UnsafeAdd(edge.Value);
                }
                valueType[] pointArray = pointList.UnsafeArray;
                int count = 0;
                if (points.count() != 0)
                {
                ...
                pointList.UnsafeAddLength(count - pointArray.Length);
            }
            if (points.count() != 0)
            {
