...
namespace fastCSharp.algorithm
{
    ...
    internal static partial class quickSort
    {
        ...
        {
            if (values == null) return default(subArray</*Type[0]*/ulong/*Type[0]*/>);
            if (count > 0)
            {
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        public unsafe static subArray</*Type[0]*/ulong/*Type[0]*/> Top/*Compare[0]*//*Compare[0]*/(/*Type[0]*/ulong/*Type[0]*/[] values, int count)
        {
            if (values == null) return default(subArray</*Type[0]*/ulong/*Type[0]*/>);
            if (count > 0)
            {
