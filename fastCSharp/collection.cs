...
namespace fastCSharp
{
    ...
    public sealed class collection<valueType> : dynamicArray<valueType>, IList<valueType>
    {
        /// <summary>
        ...
        public struct unsafer
        {
            ...
            {
                return Collection.UnsafePopExpand();
            }
            /// <summary>
            /// 弹出第一个数据
            /// </summary>
            /// <returns>第一个数据</returns>
            public valueType PopExpandReset()
            {
                valueType value = Array[Collection.StartIndex];
                Array[Collection.StartIndex] = default(valueType);
        ...
        {
            return StartIndex != EndIndex || IsFull ? array[UnsafeLastIndex] : default(valueType);
        }
        /// <summary>
        /// 最后一个元素在原始数组中的位置
        /// </summary>
        public int UnsafeLastIndex
        {
            get
            {
                return (EndIndex == 0 ? array.Length : EndIndex) - 1;
            }
        }
        ...
        /// 双向动态数组
        /// </summary>
        public collection() { array = nullValue<valueType>.Array; }
        /// <summary>
        /// 双向动态数组
        /// </summary>
        ...
        public collection(list<valueType> value)
        {
            ...
            if (EndIndex == 0) array = nullValue<valueType>.Array;
            else
            {
                array = value.array;
        ...
        public collection(valueType[] values) : this(values, false) { }
        /// <summary>
        /// 双向动态数组
        /// </summary>
        /// <param name="values">数据数组</param>
        /// <param name="isUnsafe">true表示使用原数组,false表示需要复制数组</param>
        public collection(valueType[] values, bool isUnsafe)
        {
            if (values.length() == 0) array = nullValue<valueType>.Array;
            else
            {
                if (isUnsafe)
                {
        ...
        public collection(valueType[] values, int index, int count, bool isUnsafe = false)
        {
            ...
            if (isUnsafe)
            {
                ...
                else array = values ?? nullValue<valueType>.Array; ;
            }
            else if (count == 0) array = nullValue<valueType>.Array;
            else
            {
        ...
        /// <param name="values">数据数组</param>
        /// <param name="isUnsafe">true表示可以使用原数组,false表示需要复制数组</param>
        public collection(subArray<valueType> values, bool isUnsafe = false) : this(ref values, isUnsafe) { }
        /// <summary>
        /// 双向动态数组
        /// </summary>
        /// <param name="values">数据数组</param>
        /// <param name="isUnsafe">true表示可以使用原数组,false表示需要复制数组</param>
        public collection(ref subArray<valueType> values, bool isUnsafe = false)
        {
            if (isUnsafe)
            {
                if (values.length != 0)
                {
                    ...
                    {
                        IsFull = false;
                        if ((EndIndex += values.length) == array.Length) EndIndex = 0;
                    }
                }
                else array = values.array ?? nullValue<valueType>.Array; ;
            }
            else if (values.length == 0) array = nullValue<valueType>.Array;
            else
            {
                Array.Copy(values.array, values.startIndex, array = GetNewArray(values.length), 0, values.length);
                IsFull = true;
            }
        }
        /// <summary>
        ...
        /// 枚举器
        /// </summary>
        private sealed class enumerator : IEnumerator<valueType>
        {
            /// <summary>
            /// 被枚举数组
            /// </summary>
        ...
        IEnumerator<valueType> IEnumerable<valueType>.GetEnumerator()
        {
            if (EndIndex == 0)
            {
                if (StartIndex != 0 || IsFull) return new iEnumerator<valueType>.array(this);
            }
            else if (EndIndex > StartIndex) return new iEnumerator<valueType>.array(this);
        ...
        public void Add(collection<valueType> value, int index, int count)
        {
            ...
            if (endIndex > valueCount || index < 0 || count < 0) log.Error.Throw(log.exceptionType.IndexOutOfRange);
            if (count != 0)
            {
                int length = this.Count + count;
                if (array == null || length > array.Length) setLength(length);
        ...
        public valueType PopExpand()
        {
            if (IsFull || StartIndex != EndIndex) return UnsafePopExpand();
            log.Error.Throw(log.exceptionType.IndexOutOfRange);
            return default(valueType);
        }
        /// <summary>
        ...
        /// <returns>第一个数据</returns>
        [MethodImpl((MethodImplOptions)fastCSharp.pub.MethodImplOptionsAggressiveInlining)]
        public valueType UnsafePopExpand()
        {
        ...
        /// <returns>第一个数据</returns>
        [MethodImpl((MethodImplOptions)fastCSharp.pub.MethodImplOptionsAggressiveInlining)]
        internal valueType UnsafePopExpandReset()
        {
            valueType value = array[StartIndex];
            array[StartIndex] = default(valueType);
            if (++StartIndex == array.Length) StartIndex = 0;
            IsFull = false;
            return value;
        }
        ...
        public subArray<valueType> GetFind(Func<valueType, bool> isValue)
        {
            ...
            if (length > 0)
            {
                ...
                return subArray<valueType>.Unsafe(values, 0, length);
            }
            ...
            where keyType : IEquatable<keyType>
        {
            ...
            if (Count != 0)
            {
                Dictionary<keyType, list<valueType>> newValues = dictionary<keyType>.Create<list<valueType>>(Count);
        ...
        {
            arrayType[] newValues = new arrayType[Count];
            int count = 0;
            valueType value;
            if (EndIndex > StartIndex)
            {
    ...
    public static class collection
    {
        ...
        /// <returns>双向动态数组</returns>
        [MethodImpl((MethodImplOptions)fastCSharp.pub.MethodImplOptionsAggressiveInlining)]
        public static collection<valueType> add<valueType>(this collection<valueType> collection, ref subArray<valueType> values)
        {
            if (collection != null)
            {
                collection.Add(values.array, values.startIndex, values.length);
                return collection;
            }
            return new collection<valueType>(values.array, values.startIndex, values.length, false);
        }
        /// <summary>
        /// 添加数据集合
        /// </summary>
        ...
        /// <returns>匹配值集合</returns>
        [MethodImpl((MethodImplOptions)fastCSharp.pub.MethodImplOptionsAggressiveInlining)]
        public static subArray<valueType> getFind<valueType>(this collection<valueType> collection, Func<valueType, bool> isValue)
        {
            return collection != null ? collection.GetFind(isValue) : default(subArray<valueType>);
        }
        /// <summary>
        /// 获取匹配值集合
        /// </summary>
        ...
        public static collection<valueType> each<valueType>(this collection<valueType> collection, Action<valueType> method)
        {
            if (collection.count() != 0)
            {
                ...
                valueType[] array = collection.array;
        ...
        {
            return collection != null ? collection.RangeSort(comparer, skipCount, getCount) : default(subArray<valueType>);
        }
        /// <summary>
        /// 获取排序数据分页
        /// </summary>
        ...
        {
            return collection != null ? collection.PageSort(comparer, pageSize, currentPage) : default(subArray<valueType>);
        }
        /// <summary>
        /// 获取最大值
        /// </summary>
            ...
            where valueType : IComparable<valueType>
        {
            ...
            {
                valueType value;
                if (collection.Max(iComparable<valueType>.CompareToHandle, out value)) return value;
            }
            return nullValue;
        }
        /// <summary>
        ...
        {
            return collection != null ? collection.Max(getKey, iComparable<keyType>.CompareToHandle, nullValue) : nullValue;
        }
        /// <summary>
        /// 获取最大值
        /// </summary>
        ...
        public static keyType maxKey<valueType, keyType>(this collection<valueType> collection, Func<valueType, keyType> getKey, keyType nullValue)
            where keyType : IComparable<keyType>
        {
            return collection != null ? collection.MaxKey(getKey, iComparable<keyType>.CompareToHandle, nullValue) : nullValue;
        }
        /// <summary>
        /// 获取最大键值
        /// </summary>
            ...
            where valueType : IComparable<valueType>
        {
            ...
            {
                valueType value;
                if (collection.Min(iComparable<valueType>.CompareToHandle, out value)) return value;
            }
            return nullValue;
        }
        /// <summary>
        ...
        public static valueType min<valueType, keyType>(this collection<valueType> collection, Func<valueType, keyType> getKey, valueType nullValue)
            where keyType : IComparable<keyType>
        {
            return collection != null ? collection.Min(getKey, iComparable<keyType>.CompareToHandle, nullValue) : nullValue;
        }
        /// <summary>
        /// 获取最小值
        /// </summary>
        ...
        public static keyType minKey<valueType, keyType>(this collection<valueType> collection, Func<valueType, keyType> getKey, keyType nullValue)
            where keyType : IComparable<keyType>
        {
            return collection != null ? collection.MinKey(getKey, iComparable<keyType>.CompareToHandle, nullValue) : nullValue;
        }
        /// <summary>
        /// 获取最小键值
        /// </summary>
        ...
        /// <returns>目标集合</returns>
        [MethodImpl((MethodImplOptions)fastCSharp.pub.MethodImplOptionsAggressiveInlining)]
        public static subArray<arrayType> distinct<valueType, arrayType>(this collection<valueType> collection, Func<valueType, arrayType> getValue)
        {
            return collection != null ? collection.Distinct(getValue) : default(subArray<arrayType>);
        }
        /// <summary>
        /// 连接字符串
        /// </summary>
