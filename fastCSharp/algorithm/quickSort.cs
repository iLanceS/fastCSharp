    ...
    internal static partial class quickSort
    {
            ...
            (valueType[] values, Func<valueType, valueType, int> comparer, int skipCount, int getCount)
        {
            ...
            if ((getCount = range.GetCount) != 0)
            {
                ...
                return subArray<valueType>.Unsafe(values, range.SkipCount, getCount);
            }
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        public static subArray<valueType> GetTop<valueType>(valueType[] values, Func<valueType, valueType, int> comparer, int count)
        {
            ...
            if (count > 0)
            {
                ...
                return subArray<valueType>.Unsafe(values, 0, values.Length);
            }
            return default(subArray<valueType>);
        }
        /// <summary>
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        public static subArray<valueType> Top<valueType>(valueType[] values, Func<valueType, valueType, int> comparer, int count)
        {
            if (values == null) return default(subArray<valueType>);
            if (comparer == null) log.Error.Throw(log.exceptionType.Null);
            if (count > 0)
        ...
        /// <param name="count">排序数据数量</param>
        /// <returns>排序后的数据</returns>
        private static subArray<valueType> getTop<valueType>(valueType[] values, Func<valueType, valueType, int> comparer, int count)
        {
            uint sqrtMod;
            ...
            rangeSorter<valueType> sort = new rangeSorter<valueType> { Array = newValues, Comparer = comparer, SkipCount = count - 1, GetEndIndex = count - 1 };
            sort.Sort(0, --length);
