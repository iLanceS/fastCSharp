...
//fastCSharp.Form.Forms.XXX.SubmitName='name2';
//fastCSharp.Form.Forms.XXX.OnSubmit.Add(AAA);
//fastCSharp.Form.Forms.XXX.Start({name1:{MaxLength:32,OnVerify:/^[0-9A-Z_\.]+@[0-9A-Z_\.]+$/gi,ErrorInfo:'错误'},name2:{...}});
//fastCSharp.Form.Forms.XXX.GetValue();
var fastCSharp;
(function (fastCSharp) {
    var Form = (function () {
        function Form(Parameter) {
            fastCSharp.Pub.GetParameter(this, Form.DefaultParameter, Parameter);
            fastCSharp.Pub.GetEvents(this, Form.DefaultEvents, Parameter);
        }
        ...
        Form.prototype.GetElements = function () {
            return fastCSharp.HtmlElement.$Id(this.Id).Childs(Form.IsElement).GetElements();
        };
        Form.prototype.Start = function (Elements, IsFocus) {
            ...
            for (var ElementArray = this.GetElements(), Index = -1; ++Index - ElementArray.length;) {
                ...
                fastCSharp.HtmlElement.$AddEvent(Element, ['keydown'], fastCSharp.Pub.ThisEvent(this, this.KeyDown));
                ...
                if (Type == 1 || Type == 4) {
                    ...
                    fastCSharp.HtmlElement.$AddEvent(Element, ['blur'], fastCSharp.Pub.ThisEvent(this, this.Format));
                    fastCSharp.HtmlElement.$AddEvent(Element, ['focus'], fastCSharp.Pub.ThisEvent(this, this.Focus));
                }
            }
            fastCSharp.HtmlElement.$Name('submit', fastCSharp.HtmlElement.$IdElement(this.Id)).Cursor('pointer').AddEvent('click', fastCSharp.Pub.ThisEvent(this, this.Submit));
            this.Disabled(false);
            this.Cancel(false);
            if (IsFocus) {
                try {
                ...
                if (Value != null) {
                    switch (Form.ElementType(Element)) {
                        ...
                        case 2:
                            ...
                            break;
                        case 3:
                            fastCSharp.HtmlElement.$(Element).Set('checked', Value ? Value : false);
                            break;
                        case 5:
                            fastCSharp.HtmlElement.$(fastCSharp.HtmlElement.$Name(Element.name, fastCSharp.HtmlElement.$IdElement(this.Id)).FirstElement(function (Element) { return Element.value == Value; })).Set('checked', true);
                            break;
                    }
        ...
        Form.prototype.GetElementValue = function (Element) {
            ...
            if (Type == 5)
                return fastCSharp.HtmlElement.$Name(Element.name, fastCSharp.HtmlElement.$IdElement(this.Id)).FirstElement(Form.IsChecked).value;
            if (Type == 2) {
                ...
                for (var ReturnValue = {}, Elements = this.GetElements(), Index = -1; ++Index - Elements.length;) {
                    ...
                    if (OnVerify && (!(typeof (OnVerify) == 'function' && OnVerify.toString().charAt(0) != '/' ? OnVerify(Value) : Value.toString().match(OnVerify)))) {
                        ...
                        if (this.OnError.Get().length)
                            ...
                            fastCSharp.Pub.Alert(FormElement.ErrorInfo);
                        break;
                    }
        ...
        Form.prototype.Disabled = function (IsDisabled) {
            ...
            fastCSharp.HtmlElement.$Name('submit', fastCSharp.HtmlElement.$IdElement(this.Id)).Disabled(this.IsDisabled).Cursor(Cursor);
        };
        Form.GetForms = function () {
            for (var Childs = fastCSharp.HtmlElement.$('@form').GetElements(), Index = Childs.length; --Index >= 0; this.Forms[Childs[Index].id] = new Form({ Id: Childs[Index].id }))
                ;
        };
    ...
    fastCSharp.Form = Form;
    fastCSharp.Pub.OnLoad(Form.GetForms, Form, true);
