...
(function (fastCSharp) {
    var Pub = (function () {
        ...
        Pub.ToJson = function (Value, IsIgnore, IsNameQuery, Parents) {
            ...
            if (Value != null) {
                ...
                if (Type != 'function' && (!IsIgnore || Value)) {
                    ...
                    if (typeof (Value.ToJson) == 'function')
                        return Value.ToJson(IsIgnore, IsNameQuery, Parents);
                    Parents.push(Value);
                    var Values = [];
                    if (Type == '[object Array]') {
                        for (var Index = 0; Index - Value.length; ++Index)
                            Values.push(this.ToJson(Value[Index], IsIgnore, IsNameQuery, Parents));
                        Parents.pop();
                        return '[' + Values.join(',') + ']';
                    }
                    for (var Name in Value) {
        ...
        Pub.LoadModule = function (Path) {
            if (this.IsModules[Path] == null)
                HttpRequest.Get('__PUBERROR__', { error: document.location.toString() + ' 加载了未知模块 ' + Path });
            else {
        ...
        Pub.LoadView = function (View, IsReView) {
            if (!View)
                (View = new PageView).ErrorPath = '__VIEWLOCATION__';
        ...
        Pub.LoadHash = function (PageView) {
            if (!PageView.ErrorRequest) {
                ...
                for (var Name in PageView) {
                    ...
                    if (Value == null || (!Value.__VIEWONLY__ && typeof (Value) != 'function'))
                        Data[Name] = Value;
                }
        ...
        Pub.IsModules = {};
        Pub.LoadModules = {};
        Pub.ModuleVersions = { 'htmlEditor': '4', 'ace/ace': '2', 'highcharts/highcharts': '2', 'mathJax/MathJax': '6' };
        Pub.ClientView = {};
        Pub.Functions = {};
        Pub.Identity = 0;
        return Pub;
    ...
    var HttpRequest = (function () {
            ...
            if (Request.readyState == 4) {
                ...
                try {
                    ...
                    if (IsError && Query.CallBack)
                        Query.CallBack({ __AJAXRETURN__: null, ErrorEvent: null, ErrorRequest: Query });
                }
            }
        };
        ...
        HttpRequest.prototype.MakeXMLHttpRequest = function () {
            ...
            if (!Pub.IE && Info.Method === 'GET' && !Info.UserName && !Info.IsOnLoad) {
                Info.RetryCount = 2;
                Pub.AppendJs(Url, fastCSharp.Loader.Charset, null, (Pub.AjaxAppendJs = Info).GetOnError(this));
            }
            else {
        ...
        HttpRequest.Get = function (Url, Send, CallBack) {
            if (Send === void 0) { Send = null; }
            if (CallBack === void 0) { CallBack = null; }
        ...
        HttpRequest.CheckError = function (Value, ErrorInfo) {
            ...
            if (Value.ErrorRequest) {
                if (ErrorInfo)
                    fastCSharp.Pub.Alert(ErrorInfo);
                return false;
            }
            return true;
        };
            ...
            if ((lineno || message != 'Script error.') && (lineno != 1 || message != 'Error loading script')) {
                if (lineno == 1) {
                    ...
                    if (Ajax.RetryCount && document.location.origin + Ajax.Url == filename && --Ajax.RetryCount) {
                        Pub.AppendJs(Ajax.Url, fastCSharp.Loader.Charset, null, Ajax.GetOnError(null));
                        return;
                    }
                }
        ...
        Declare.prototype.ParameterMany = function (Event) {
            if (!Event.IsGetOnly) {
                ...
                if (Element) {
                    ...
                    Parameter.Event = Event;
                    if (Parameter.Id) {
                        var Value = Declare.Declares[this.fastCSharpName][Parameter.Id];
                        if (Value) {
                            Value.Reset(Parameter, Element);
                            return Value;
                        }
                        Declare.Declares[this.fastCSharpName][Parameter.Id] = (Value = this.NewDeclare(Parameter));
                        return Value;
                    }
                }
            }
