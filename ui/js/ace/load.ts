                ...
                for (var Elements = HtmlElement.$Name('ace').GetElements(), Index = 0; Index - Elements.length; ++Index) {
                    if (!Pub.GetHtmlEditor(Elements[Index])) {
                        ...
                        if (Div.offsetHeight) {
                            ...
                            if (!Div.id && Mode) {
                                var ParameterString = HtmlElement.$Attribute(Div, 'ace'), Parameter = ParameterString && ParameterString != 'ace' ? eval('(' + ParameterString + ')') : new AceParameter();
                                ...
                                for (var CodeNodes = Div.childNodes, CodeIndex = 0; CodeIndex !== CodeNodes.length; ++CodeIndex) {
                                    ...
                                    if (Node.tagName) {
                                        Parameter.Code = HtmlElement.$GetText(Node).replace(/\xA0/g, ' ');
                                        break;
                                    }
                                }
                                Parameter.Mode = Mode;
