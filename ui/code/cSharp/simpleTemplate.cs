...
namespace fastCSharp.code
{
    ...
    /// 自定义简单组合模板
    /// </summary>
    [auto(Name = "自定义模板", IsAuto = true, IsTemplate = false)]
    internal sealed class simpleTemplate : IAuto
    {
        /// <summary>
        /// 自定义模板相对项目路径
        /// </summary>
        public static readonly string DefaultTemplatePath = (@"template\").pathSeparator();
        /// <summary>
        /// 安装入口
        /// </summary>
        ...
        private sealed class replace
        {
            ...
            /// 替换的目标组合集合
            /// </summary>
            public subArray<subString>[] values;
            /// <summary>
            /// 当前组合索引
            /// </summary>
            public int index;
...
", StringComparison.Ordinal);
            if (index != -1)
            {
                ...
                Dictionary<subString, replace> keys = dictionary.CreateSubString<replace>();
                replace replace;
...
", StringComparison.Ordinal))
                    {
                        keys[subString.Unsafe(keyValue, 0, index)] = replace = new replace
                        {
                            values = subString.Unsafe(keyValue, ++index, keyValue.Length - index - 4).Split(';').GetArray(value => value.Split(','))
                        };
                ...
                do
                {
                    ...
                    for (index = replaces.Length; --index >= 0; )
                    {
                        ...
                        subArray<subString> replaceCode = replace.values[replace.index];
